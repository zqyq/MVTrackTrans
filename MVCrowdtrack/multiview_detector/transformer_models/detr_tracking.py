...
class DETRTrackingBase(nn.Module):
    ...
    def add_track_queries_to_targets(self, targets, prev_indices, prev_out, add_false_pos=True):
        ...
        if min_prev_target_ind:
            ...
            #num_prev_target_ind = min_prev_target_ind-1
        ...
        for i, (target, prev_ind) in enumerate(zip(targets, prev_indices)):
            ...
            prev_track_ids = target['prev_target']['track_ids'].squeeze()[prev_target_ind]

            # match track ids between frames
            target_ind_match_matrix = prev_track_ids.unsqueeze(dim=1).eq(target['track_ids']).to(device)
    ...
    def forward(self, samples: NestedTensor,cal, targets: list = None, prev_features=None):
            ...
            # if True:
                ...
                with backprop_context():
                    if 'prev_prev_image' in targets[0]:
                        ...
                        prev_out, _, prev_features, _, _ = super().forward([t['prev_image'] for t in targets],cal)
        ...
        out, targets, features, memory, hs  = super().forward(samples, cal,targets,prev_features)
