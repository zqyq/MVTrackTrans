    ...
    def test(self,data_loader,args,postprocessors):
        ...
        for i, (samples, targets,cal) in enumerate(data_loader):
            ...
            if len(confirmed) > 0 and pre_memories is not None:
                ...
                strack_pool_xy[..., 0] /= 1200
                strack_pool_xy[..., 1] /= 800
                strack_pool_xy = strack_pool_xy.unsqueeze(0)

                with torch.no_grad():
                    out, pre_memories = self.model(features, pre_cts=strack_pool_xy,pre_memories=pre_memories,img_features=img_features)
            else:
                with torch.no_grad():
            ...
            #plt.colorbar()
            plt.axis('off')
            plt.savefig(f"test/{frame_id + 3297}_heatmap.png", bbox_inches='tight', pad_inches=0)
            plt.close()

            xy_e, scores = decode.decoder(
                heatmap.sigmoid() ,K=100
            )
            xy_e*=4
            ...
            mota_pred_list.extend([[frame_id+3297, s.track_id, -1, -1, -1, -1, s.score.item()]
                                        + s.xy.tolist() + [-1]
