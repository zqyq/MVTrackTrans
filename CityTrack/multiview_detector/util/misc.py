...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    """
    tensor_list: List[Tensor]，其中每个 Tensor 形状为 (b, num_cameras, C, H, W)
    """

    # 检查每个 tensor 的维度
    if tensor_list[0].ndim != 5:
        raise ValueError("Expected 5D tensors in list: (b, num_cameras, C, H, W)")

    batch_size = len(tensor_list)
    num_cameras = tensor_list[0].shape[1]  # 第二维是 num_cameras

    # 计算最大 channels, height, width
    max_c = max(t.shape[2] for t in tensor_list)
    max_h = max(t.shape[3] for t in tensor_list)
    max_w = max(t.shape[4] for t in tensor_list)

    dtype = tensor_list[0].dtype
    device = tensor_list[0].device

    # 初始化 padded tensor 和 mask
    tensor = torch.zeros((batch_size, num_cameras, max_c, max_h, max_w), dtype=dtype, device=device)
    mask = torch.ones((batch_size, max_h, max_w), dtype=torch.bool, device=device)

    for i, img in enumerate(tensor_list):  # 遍历每个样本
        for cam_idx in range(num_cameras):
            c, h, w = img[0][cam_idx].shape # 获取 C, H, W
            tensor[i, cam_idx, :c, :h, :w] = img[0][cam_idx]  # 这里确保形状对齐
            mask[i, :h, :w] = False  # False 表示有效区域，True 表示 padding 区域
