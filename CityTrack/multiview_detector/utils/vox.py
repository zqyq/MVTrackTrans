...
import multiview_detector.utils.geom
import multiview_detector.utils.basic
...
class VoxelUtil:
    ...
    def Ref2Mem(self, xyz, Y, Z, X, assert_cube=False):
        ...
        xyz = multiview_detector.utils.geom.apply_4x4(mem_T_ref, xyz)
        return xyz
    ...
    def Mem2Ref(self, xyz_mem, Y, Z, X, assert_cube=False):
        ...
        xyz_ref = multiview_detector.utils.geom.apply_4x4(ref_T_mem, xyz_mem)
        return xyz_ref
    ...
    def get_mem_T_ref(self, B, Y, Z, X, assert_cube=False, device='cuda'):
        ...
        mem_T_ref = multiview_detector.utils.basic.matmul2(mem_T_center, center_T_ref)
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_refA, camB_T_refA, Y, Z, X, assert_cube=False, xyz_refA=None,
        ...
        if xyz_refA is None:
            xyz_memA = multiview_detector.utils.basic.gridcloud3d(B, Y, Z, X, norm=False, device=pixB_T_refA.device)
            xyz_refA = self.Mem2Ref(xyz_memA, Y, Z, X, assert_cube=assert_cube)
        xyz_camB = multiview_detector.utils.geom.apply_4x4(camB_T_refA, xyz_refA)
        xyz_pixB = multiview_detector.utils.geom.apply_4x4(pixB_T_refA, xyz_refA)
        ...
        y_pixB, x_pixB = multiview_detector.utils.basic.normalize_grid2d(y, x, H, W)
        ...
        xyz_pixB = xyz_pixB.to(rgb_camB.device)
        #values = F.grid_sample(rgb_camB, xyz_pixB, align_corners=False, mode=mode)
        values = F.grid_sample(rgb_camB, xyz_pixB, align_corners=False, mode=mode)
        valid_mem = valid_mem.to(values.device)
    ...
    def warp_tiled_to_mem(self, rgb_tileB, pixB_T_ref, camB_T_ref, Y, Z, X, DMIN, DMAX, assert_cube=False, z_sign=1):
        ...
        xyz_memA = multiview_detector.utils.basic.gridcloud3d(B, Y, Z, X, norm=False, device=pixB_T_ref.device)

        xyz_ref = self.Mem2Ref(xyz_memA, Y, Z, X, assert_cube=assert_cube)

        xyz_camB = multiview_detector.utils.geom.apply_4x4(camB_T_ref, xyz_ref)
        z_camB = xyz_camB[:, :, 2]

        # rgb_tileB has depth=DMIN in tile 0, and depth=DMAX in tile D-1
        z_tileB = (D - 1.0) * (z_camB - float(DMIN)) / float(DMAX - DMIN)

        xyz_pixB = multiview_detector.utils.geom.apply_4x4(pixB_T_ref, xyz_ref)
        normalizer = torch.unsqueeze(xyz_pixB[:, :, 2], 2)
        EPS = 1e-4
        normalizer[normalizer <= 0] = normalizer[normalizer <= 0].clamp(max=-EPS)
        normalizer[normalizer >= 0] = normalizer[normalizer >= 0].clamp(min=EPS)
        ...
        z_tileB, y_pixB, x_pixB = multiview_detector.utils.basic.normalize_grid3d(z_tileB, y, x, D, H, W)
