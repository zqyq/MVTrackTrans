...
class frameDataset(VisionDataset):
    def __init__(self, base, train=True, transform=ToTensor(), target_transform=ToTensor(),
        ...
        self.pid_dict = {}
        if self.is_train:
            frame_range = range(0, 1948)
        else:
            frame_range = range(1948, self.num_frame)
    ...
    def get_bev_gt(self, mem_pts, pids):
        ...
        for pt, pid in zip(mem_pts[0], pids):
            ...
            if x < 0 or x >= 640 or y < 0 or y >= 768:
                continue
        ...
        center = torch.zeros((1, 384, 320), dtype=torch.float32)
        offset = torch.zeros((2, 384, 320), dtype=torch.float32)

        for pts, pid in zip(mem_pts[0], pids):
            x_idx, y_idx = int(pts[0]/2), int(pts[1]/2)
            #print(x_idx,y_idx)
            ct = pts[:2]
            ct_int = ct.int()
            if x_idx < 0 or x_idx >= 320 or y_idx < 0 or y_idx >= 384:
                continue


            basic.draw_umich_gaussian(center[0], [x_idx, y_idx], 3)
        ...
        for pt_idx, (pid, wh) in enumerate(zip(img_pids, size_pts)):
            for idx, pt in enumerate((foot_pts[pt_idx], )):  # , center_pts[pt_idx], head_pts[pt_idx])):
                if pt[0] < 0 or pt[0] >= W or pt[1] < 0 or pt[1] >= H:
                    continue
                basic.draw_umich_gaussian(center[idx], pt.int(), 3)
    ...
    def __getitem__(self, index):
        ...
        target = {
            ...
            'orig_size': torch.tensor([768,640]),
            'size': torch.tensor([768,640]),
            'boxes': None,
            'track_ids': None,
        ...
        if frame!=1948:
            ...
            candidate_prev_frames = [f for f in [frame - 1] if f >= 0]
        ...
        prev_worldgrid_pts[..., 0] /= 640
        prev_worldgrid_pts[..., 1] /= 768
        ...
        if len(track_pts) > 0:
            ...
            track_pts = torch.empty(0, 2, device=worldgrid_pts.device)
            track_offsets = torch.empty(0, 2, device=worldgrid_pts.device)
        ...
        for cam in range(self.num_cam):
            ...
            target[f'view{cam}'] = {
                'image_id': torch.tensor(frame),
                'size': torch.tensor([1520, 2704]),
                'orig_size': torch.tensor([1520, 2704]),
